...
//	alert("2 = libArray after clean: "+libArray[libArray.length-1]);
...
function arrayClean(arr) {
	...
	for(var i in arr) {
		if (arr[i][9]) {
//			Create full library entries from shorter snippets
//			if (arr[i][2] == "HSV") {
//			} else if (arr[i][2] == "RGB") {
//			} else if (arr[i][2] == "HEX") {
//			} else
...
function libraryToggle(event) {
	...
	var bottomPosition0 = 74;	// actually 74, not 66, some equations may require (c-8) or (c+8)
	var bottomPosition1 = 8;
	var originalBottom = parseInt(document.defaultView.getComputedStyle(libraryItem, null).getPropertyValue("bottom"));
	var moveHandler = function(a, c, s, f){ libraryItem.style.bottom = c+"px"; libraryToggleItem.style.top = (-10+((c-8)/6))+"px"; stackItem.style.top = (172-(c-8))+"px"; };
